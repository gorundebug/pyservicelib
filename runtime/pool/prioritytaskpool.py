...
from pyservicelib.runtime.pool import PriorityTaskPool
...
class PriorityTaskPoolImpl(PriorityTaskPool):
    ...
    async def _executor(self):
        while True:
            priority, task, args, kwargs = await self._task_queue.get()
            if task is None:
                break
            try:
    ...
    async def stop(self, ctx: Context):
        for _ in self._executors:
            await self._task_queue.put((float('inf'), None, None, None))
