...
def runtime_to_stream_app(app: "ServiceApp") -> StreamApp:
    ...
    for li in app._runtime_links:
        ...
        link_config = config.get_link(li.from_id, li.to_id)
        if link_config is not None:
            source_config = config.get_stream_config_by_id(li.from_id)
            is_outgoing = (
                svc_cfg is not None and source_config.id_service == svc_cfg.id
            )
            if cs == CallSemantics.FunctionCall and is_outgoing:
                ...
                if cs == CallSemantics.PriorityTaskPool:
                    priority = (
                        ...
                        if is_outgoing
                        else link_config.income_priority
                    )
