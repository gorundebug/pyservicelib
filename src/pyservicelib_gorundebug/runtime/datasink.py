...
class OutputDataSink(DataSink, ABC):
    ...
    def __init__(self, connector_id: int, env: ServiceExecutionEnvironment):
        ...
        self._name = env.config.get_data_connector_by_id(connector_id).name
        self._endpoints = {}
...
class DataSinkEndpoint(SinkEndpoint):
    ...
    def __init__(self, data_sink: DataSink, id_endpoint: int):
        ...
        if path:
            ...
            http_method_type = getattr(
                endpoint_config, "http_method_type", None,
            )
            self._transport_metrics = TransportRequestMetrics.http_client(
                data_sink.environment.metrics,
                method=(
                    getattr(http_method_type, "value", None) or "UNKNOWN"
                ),
                route=f"http://{authority}{route}",
                server_address=host,
                server_port=port,
            )
        elif method_name:
