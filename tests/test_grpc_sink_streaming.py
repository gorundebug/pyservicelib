...
async def test_client_streaming_single_message():
    ...
    async def client_fn(metadata=(), timeout=None):
...
async def test_client_streaming_begin_request_failure():
    ...
    async def client_fn2(metadata=(), timeout=None):
        assert timeout is None
        nonlocal retried
        retried = True
...
async def test_client_streaming_grpc_call_failure_drops_reservation():
    ...
    async def failing_client_fn(metadata=(), timeout=None):
        assert timeout is None
...
def test_request_timeout_propagates_remaining_request_deadline():
...
def test_request_timeout_clamps_expired_deadline_to_zero():
    token = request_deadline.set(
        datetime.now(timezone.utc) - timedelta(seconds=1)
    )
    try:
        assert _request_timeout() == 0.0
    finally:
        request_deadline.reset(token)
